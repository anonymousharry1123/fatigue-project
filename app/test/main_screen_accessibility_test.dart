...
void main() {
  ...
  for (final brightness in Brightness.values) {
    group(brightness.name, () {
      ...
      testWidgets(
        'Shell and Forecast/Insights navigation fit 320px at 2x text',
        (tester) async {
        ...
        (tester) async {
          ...
          expect(
            find.bySemanticsLabel(
              RegExp('Average estimated Energy:.*out of 100'),
            ),
            findsWidgets,
          );
          await tester.drag(
      ...
      testWidgets(
        'Insights trend reads dates, units, and missing observations',
        (tester) async {
          final semantics = tester.ensureSemantics();
          final controller = await _controller();
          addTearDown(controller.dispose);
          ...
          expect(
            find.bySemanticsLabel(RegExp('Sleep: 0.0 hours')),
            findsNothing,
          );
      ...
      for (final textScale in [1.0, 2.0]) {
        testWidgets(
          'Today fits 320px at ${textScale}x text throughout the page',
          (tester) async {
            final controller = await _controller();
            addTearDown(controller.dispose);
            await _pumpScreen(
